  ...
  return (
    <footer className="w-full py-20 px-[var(--spacing-sizing-page-padding-padding-global)] bg-color-schemes-color-scheme-1-background">
      <div className="flex flex-col max-w-[var(--spacing-sizing-container-container-large)] mx-auto gap-20 w-full">
        {/* Top row with logo, navigation and social icons */}
        <div className="flex items-center gap-8 w-full">
          ...
          <nav className="flex items-center gap-8">
            {navLinks.map((link, index) => (
              <Link
                key={index}
                className="font-text-small-semi-bold text-[length:var(--text-small-semi-bold-font-size)] leading-[var(--text-small-semi-bold-line-height)] text-[color:var(--color-schemes-color-scheme-1-text)] tracking-[var(--text-small-semi-bold-letter-spacing)] whitespace-nowrap"
                to={link.path}
              >
                {link.text}
              </Link>
        ...
        {/* Bottom row with divider, copyright and terms */}
